...
exports.sourceNodes = async (api, pluginOptions) => {
  ...
  let files = await gcpBucket.getFiles({
  ...
  if (files[0].length < 1) {
    ...
    let fileList = [];
    files[0].forEach(f => {
      fileList.push(f.getMetadata().then(item => {
...
let example = {
  kind: 'storage#object',
  id: 'tf-state-laaso-cluster/terraform/state/default.tfstate/1566416769859377',
  selfLink: 'https://www.googleapis.com/storage/v1/b/tf-state-laaso-cluster/o/terraform%2Fstate%2Fdefault.tfstate',
  ...
  mediaLink: 'https://www.googleapis.com/download/storage/v1/b/tf-state-laaso-cluster/o/terraform%2Fstate%2Fdefault.tfstate?generation=1566416769859377&alt=media',
  crc32c: '6H2dzQ==',
