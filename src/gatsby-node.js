...
exports.sourceNodes = async (api, pluginOptions) => {
    ...
    const {name, gcpTokenFile, gcpBucketName, storageDirectory} = pluginOptions
    ...
    gcpBucket.getFiles({directory: storageDirectory || `/`}, (err, files) => {
        if(err) {
            reporter.panic(`
            ...
            files.forEach(f => {
                f.getMetadata().then(
                    m => reporter.info(`gatsby-source-gcp-storage: processing file ${m[0].name}`)
                )
            })
        }
    })        
